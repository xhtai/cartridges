...
#'
#' This method is intended only as a rough search for
#' a starting pixel location to be used by \code{gridSearch()}.
#' An appropriate center would fit the ring in the image
#' well, and the goal is to find a center producing the
#' longest continuous string of fitted small coefficients where the
#' ring is expected to be. An approximate location of the
#' ring is 450-1000 pixels from the center, so only the
#' subset of basis functions corresponding to this distance
#' is recommended.
#'
#' Along an input row (recommended rows are 940 and 980),
#' we consider 201 possible center locations from columns
#' 860 to 1060. Then after selecting an appropriate column,
#' we consider 201 possible rows (100 above and 100 below
#' the input row). We recommend use of a further
#' subset of basis functions (those with 8 pixels in each
#' basis function, since these are the most common) for speed.
#' Also, instead of fitting each basis function (computing
#' the mean of the pixel values) and searching for the
#' longest string of small values, we convert the images from
#' 256-grayscale to binary values, with values from 0 to 127
#' taking the value 0 and those from 128 to 255 taking the
#' value 1. Now at each possible center, we compute the sum
#' of pixel values of each basis function, and find the
#' center resulting in the longest continuous string of zeros.
#'
#'
#' @param image image that rough center is to be
#' determined for
#' @param basis subset of basis functions of the
#' appropriate dimension (1919 x 1919). Recommended
#' restrictions are basis functions with
#' distances from center of 450-1000
#' pixels, and with 8 pixels in each basis function.
#' @param startingRow row to be searched
#'
#' @return A data frame with 402 rows,
#' each row with the coordinates of the center
#' considered, and the maximum number of continuous
#' zeros.
#'
#' @examples
...
#'
#' At a grid of potential centers, fit basis functions
#' corresponding to the ring
#' (approximately 450-1000 pixels from the center)
#' to the image, and calculate the total residual sum of
#' squares of the fit. Small values indicate a better
#' fit and a more appropriate center.
#'
#' @param image image that center is to be
#' determined for
#' @param basis subset of basis functions of the
#' appropriate dimension (1919 x 1919), corresponding
#' approximately to the outer ring. Recommended
#' distances from center are 450-1000 pixels.
#' @param startingRow starting row to be searched
#' @param startingColumn starting column to be searched
#' @param plot logical value indicating whether a surface
#' plot of total RSS values is desired
#'
#' @return A data frame with the ij coordinates of the
#' center considered, and the total residual sum of
#' squares of the fit.
#'
#' @examples
#' data(LL1_3,basis1919)
...
gridSearch<-function(image,basis,startingRow,startingColumn,plot){
    ...
    while ((tmp$centeri %in% c(min(RSSdtf$centeri),max(RSSdtf$centeri)) | tmp$centerj %in% c(min(RSSdtf$centerj),max(RSSdtf$centerj)))){
        ...
        if (multiplier>10) error("Center not found after long search")
    }
    if (plot==TRUE){
...
#'
#' Produces a surface plot of the total residual sum
#' of squares of all the centers tested
#'
#' @param RSS data frame of RSS values, in the format
#' produced by \code{gridSearch()}
